...
class MyModel:
    ...
    def __init__(self, path, dataset_name, model_name, task, labels, cased):
        ...
        Args:
            ...
            model_name: The transformer's name (currently only 'bert','distilbert' and 'roberta' are available)
    ...
    def __load_model__(self):
        ...
        # added RoBERTa
            ...
            # cs is missing in RoBERTa
            self.tokenizer = RobertaTokenizerFast.from_pretrained('roberta-base')
            if self.task.lower() == 'single_label':
                from transformers import \
                    RobertaForSequenceClassification as transformer_model
            else:
                from myTransformer import \
                    RobertaForMultilabelSequenceClassification as \
                        transformer_model

        if self.task.lower() == 'single_label':
            model = transformer_model.from_pretrained(self.path + self.dataset_name, output_attentions=True, output_hidden_states=True)
        else:
            model = transformer_model.from_pretrained(self.path + self.dataset_name, num_labels=self.labels, output_attentions=True, output_hidden_states=True)
        ...
        training_arguments = TrainingArguments(evaluation_strategy='epoch', save_strategy='epoch',
    ...
    def __get_additional_info_from_trainer__(self):
        ...
        if self.model_name.lower() == 'albert':
            ...
            # they are shared across layers
            keys = self.trainer.model.albert.encoder.albert_layer_groups[0].albert_layers[
                0].attention.key.weight.cpu().detach().numpy()
            queries = self.trainer.model.albert.encoder.albert_layer_groups[0].albert_layers[
                0].attention.query.weight.cpu().detach().numpy()
        ...
        # added RoBERTa
            ...
            for i in range(self.layers):
                self.key_list.append(
                    self.trainer.model.roberta.encoder.layer[i].attention.self.key.weight.cpu().detach().numpy())
                self.query_list.append(
                    self.trainer.model.roberta.encoder.layer[i].attention.self.query.weight.cpu().detach().numpy())
