...
class Optimus:
    """Optimus class is an easy-to-use tool for interpreting locally transformer models (currently BERT,DistilBERT and RoBERTa). Check the github's repo for a few examples!"""
    ...
    def __init__(self, model, tokenizer, label_names, task, set_of_instance=None):
        ...
        if len(self.set_of_instance) > 1:
            ...
            print(
                'No best setup calculated as you did not provided a set of instances. You will not be able to use max_across later')
    ...
    def __export_model_information__(self):
        ...
        # added RoBERTa
            ...
            for i in range(self.layers):
                self.key_list.append(
                    self.model.model.roberta.encoder.layer[i].attention.self.key.weight.cpu().detach().numpy())
                self.query_list.append(
                    self.model.model.roberta.encoder.layer[i].attention.self.query.weight.cpu().detach().numpy())
        ...
        # added ALBERT
            ...
            keys = self.model.model.albert.encoder.albert_layer_groups[0].albert_layers[
                0].attention.key.weight.cpu().detach().numpy()
            queries = self.model.model.albert.encoder.albert_layer_groups[0].albert_layers[
    ...
    def explain(self, instance, mode='baseline', level='token', raw_attention='A'):
        ...
        if raw_attention not in ['A', 'A*']:
            print(
                'Please choose raw attention among: A (for attention after softmax), A* (for attention before softmax)')
            return
