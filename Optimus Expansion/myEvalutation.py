...
class MyEvaluation:
    def __init__(self, label_names, predict, sentence_level, evaluation_level_all=True, tokenizer=None):
    ...
    def clear_states(self):
        """
            This function resets the state memory used by truthfulness metric
        """
        self.saved_state = {}
    ...
    def nzw(self, interpretation, tweaked_interpretation, instance, prediction, tokens, hidden_states, t_hidden_states,
            rationales):
        """
            This function evaluates an interpretation using the nzw/non-zero-weights (complexity) metric
        Args:
            ...
            hidden_states: The hidden states regarding that instance extracted by the model
            t_hidden_states: The hidden states regarding the tweaked instance extracted by the model (useful for robustness)
    ...
    def faithfulness(self, interpretation, tweaked_interpretation, instance, prediction, tokens, hidden_states,
                     t_hidden_states, rationales):
        """
            This function evaluates an interpretation using the faithdulness score (F) metric
        Args:
            interpretation: The interpretations of the instance's prediction
            tweaked_interpretation: The interpretations of the tweaked instance's prediction (useful for robustness)
            instance: The input sequence to be fixed from tokenized to original
        ...
        for label in range(len(self.label_names)):
            ...
            if predicted_labels[label] >= 0.5 or self.evaluation_level_all:
                ...
                if self.sentence_level:
                    ...
                    do_all_tokens_start_with_ = all(
                        token.startswith('▁') for token in temp_tokens if token != '[CLS]' and token != '[SEP]')
                    ...
                    # if the model is gpt-2
                    # elif '<|endoftext|>' in tokens:
    ...
    def truthfulness(self, interpretation, tweaked_interpretation, instance, prediction, tokens, hidden_states,
                     t_hidden_states, rationales):
        """
            This function evaluates an interpretation using the truthfulness metric
        Args:
            interpretation: The interpretations of the instance's prediction
            tweaked_interpretation: The interpretations of the tweaked instance's prediction (useful for robustness)
            instance: The input sequence to be fixed from tokenized to original
            ...
            if predicted_labels[label] >= 0.5 or self.evaluation_level_all:
                ...
                for token in range(0, my_range):
                    ...
                    if self.sentence_level:
                        ...
                        # if all tokens start with _ -> the model is ALBERT
                        do_all_tokens_start_with_ = do_all_tokens_start_with_ = all(
                            token.startswith('▁') for token in temp_tokens if token != '[CLS]' and token != '[SEP]')
    ...
    def faithful_truthfulness(self, interpretation, tweaked_interpretation, instance, prediction, tokens, hidden_states,
                              t_hidden_states, rationales):
        """
            This function evaluates an interpretation using the Faithful Truthfulness score (FT) metric
        Args:
            interpretation: The interpretations of the instance's prediction
            tweaked_interpretation: The interpretations of the tweaked instance's prediction (useful for robustness)
            instance: The input sequence to be fixed from tokenized to original
    ...
    def faithful_truthfulness_penalty(self, interpretation, tweaked_interpretation, instance, prediction, tokens,
                                      hidden_states, t_hidden_states, rationales):
        """
            This function evaluates an interpretation using the Ranked Faithful Truthfulness score (RFT) metric
        Args:
            interpretation: The interpretations of the instance's prediction
            tweaked_interpretation: The interpretations of the tweaked instance's prediction (useful for robustness)
            instance: The input sequence to be fixed from tokenized to original
            ...
            if predicted_labels[label] >= 0.5 or self.evaluation_level_all:  # :| h gia ola?????? pfff
                ...
                for token in range(0, my_range):
                    ...
                    if sign == 'positive':
                        ...
                        score += (-1) * abs(preds[1][label] - preds[0][label]) / value_order[
                            abs(interpretation[label][token])]  # Penalty
                avg_diff.append(score)
            else:
                avg_diff.append(np.average([]))
        return avg_diff

    def robustness(self, interpretation, tweaked_interpretation, instance, prediction, tokens, hidden_states,
                   t_hidden_states, rationales):
        """
            This function evaluates an interpretation using the Robustness metric
        Args:
            interpretation: The interpretations of the instance's prediction
            tweaked_interpretation: The interpretations of the tweaked instance's prediction (useful for robustness)
            instance: The input sequence to be fixed from tokenized to original
    ...
    def auprc(self, interpretation, tweaked_interpretation, instance, prediction, tokens, hidden_states,
              t_hidden_states, rationales):
        """
